...
function getEnvFile(suffix) {
  ...
  let fileName = helpers.root(`Source/environments/environment${suffix}.ts`);
  if (fs.existsSync(fileName)) {
    return fileName;
  } else if (fs.existsSync(fileName = helpers.root('Source/environments/environment.ts'))) {
...
function ngcWebpackSetup(prod, metadata) {
  ...
  if (environment) {
    ngcWebpackPluginOptions.hostReplacementPaths = {
      [helpers.root('Source/environments/environment.ts')]: environment
    }
  }
